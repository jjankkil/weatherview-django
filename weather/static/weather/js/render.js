...
export function applyLabels() {
    ...
    if (dom.trendHeader)
        setText(dom.trendHeader, L.historyTitle);
    setText(dom.settingsSave, L.save);
    setText(dom.settingsCancel, L.cancel);
    setText(dom.settingsTitle, L.settingsTitle);
    if (dom.cookieBannerText)
        setText(dom.cookieBannerText, L.cookieBanner);
    if (dom.cookieBannerOk)
        setText(dom.cookieBannerOk, L.cookieBannerOk);
...
export function populateStations(stations) {
    ...
    if (mruStations.length > 0) {
        ...
        for (const s of mruStations) {
            ...
            if (s.id === state.currentStationId)
                opt.selected = true;
        ...
        for (const s of stations) {
            ...
            opt.value = s.id;
            opt.textContent = s.formatted_name;
            if (!currentInMru && s.id === state.currentStationId)
                opt.selected = true;
