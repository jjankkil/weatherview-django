"use strict";
import { initCamera, carousel, lightbox, carouselGoTo, lightboxGoTo, openLightbox, closeLightbox, } from "./camera.js";
import { state, forecastCarousel, FORECAST_PAGE_SIZE } from "./state.js";
import { dom, labels, applyLabels, setVisible, openSettings, closeSettings, openStationSearch, closeStationSearch, renderStationSearchResults, populateStations, forecastGoTo, } from "./render.js";
import { fetchSettings, saveSettings, fetchStations, fetchWeather, fetchStationHistory, clearCountdown, startNextUpdateDisplay, } from "./api.js";
...
function initEvents() {
    dom.stationSelect.addEventListener("change", () => {
        ...
        saveSettings({
            ...
            current_station_name: opt.textContent,
        });
        fetchWeather(id);
    });
    ...
    dom.langBtn.addEventListener("click", (e) => {
        ...
        dom.langDropdown.setAttribute("aria-expanded", String(dom.langDropdown.classList.contains("open")));
    });
    ...
    document.addEventListener("click", (e) => {
        if (!(e.target instanceof Node) || !dom.langDropdown.contains(e.target)) {
            dom.langDropdown.classList.remove("open");
            dom.langDropdown.setAttribute("aria-expanded", "false");
        }
    });
    dom.forecastPrev.addEventListener("click", () => {
        if (forecastCarousel.index > 0)
            forecastGoTo(Math.max(0, forecastCarousel.index - FORECAST_PAGE_SIZE));
    });
    dom.forecastNext.addEventListener("click", () => {
        if (forecastCarousel.index + FORECAST_PAGE_SIZE <
            forecastCarousel.items.length)
    ...
    dom.carouselNext.addEventListener("click", () => {
        if (carousel.index < carousel.slides.length - 1)
            carouselGoTo(carousel.index + 1);
    });
    ...
    dom.lightboxPrev.addEventListener("click", () => {
        if (lightbox.index > 0)
            lightboxGoTo(lightbox.index - 1);
    });
    dom.lightboxNext.addEventListener("click", () => {
        if (lightbox.index < carousel.slides.length - 1)
            lightboxGoTo(lightbox.index + 1);
    });
    let lightboxTouchX = 0;
    dom.lightboxTrack.addEventListener("touchstart", (e) => {
    ...
    dom.lightboxTrack.addEventListener("touchend", (e) => {
        ...
        if (dx < -40 && lightbox.index < carousel.slides.length - 1)
            lightboxGoTo(lightbox.index + 1);
        else if (dx > 40 && lightbox.index > 0)
            lightboxGoTo(lightbox.index - 1);
    }, { passive: true });
    let touchStartX = 0;
    ...
    dom.carouselTrack.addEventListener("touchend", (e) => {
        ...
        if (dx < -40 && carousel.index < carousel.slides.length - 1)
            carouselGoTo(carousel.index + 1);
        else if (dx > 40 && carousel.index > 0)
            carouselGoTo(carousel.index - 1);
    }, { passive: true });
    ...
    dom.stationSearchModal.addEventListener("click", (e) => {
        if (e.target === dom.stationSearchModal)
            closeStationSearch();
    });
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape") {
            if (!dom.lightbox.classList.contains("hidden"))
                closeLightbox();
            else if (!dom.stationSearchModal.classList.contains("hidden"))
                closeStationSearch();
            else
                closeSettings();
        }
...
function initCookieBanner() {
    if (!dom.cookieBanner)
        return;
    if (localStorage.getItem(_CONSENT_KEY))
        return;
...
document.addEventListener("visibilitychange", () => {
    if (document.hidden) {
        ...
        if (remaining <= 0) {
            ...
            if (state.currentStationId)
                fetchWeather(state.currentStationId, true);
        }
        else {
