...
import { dom, labels, showError, hideError, renderWeather, populateStations, } from "./render.js";
...
export async function fetchSettings() {
    try {
        ...
        if (data.current_station_id)
            state.currentStationId = data.current_station_id;
        if (data.show_camera !== undefined)
            state.showCamera = data.show_camera;
        if (data.show_history !== undefined)
            state.showHistory = data.show_history;
        if (data.history_hours !== undefined)
            state.historyHours = data.history_hours;
    }
    catch (_) {
        /* ignore */
    }
...
export async function fetchWeather(stationId, fresh = false) {
    ...
    try {
        const url = fresh
            ? `/api/station/${stationId}/?refresh=1`
        ...
        if (!r.ok) {
            ...
            if (!msg)
                msg = labels().serviceError.replace("{code}", r.status);
            showError(msg);
            return;
        }
        const data = await r.json();
        ...
        if (state.showCamera)
            showCameraForStation(stationId);
