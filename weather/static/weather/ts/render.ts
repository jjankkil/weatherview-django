...
export const LABELS = {
    ...
    rainSum24h: "Sade 24 h",
    rainSumTotal: "Sade {h} h",
    cameraLoaded: "Ladattu",
    cameraDirection: "Suunta",
    cameraImageUnavailable: "Kuva ei saatavilla",
    stationDistanceLabel: "Etäisyys sijaintiisi:",
    stationDistanceAccuracy: "(tarkkuus ±{d})",
    cameraDistanceLabel: "Kameran etäisyys asemasta:",
    ...
    serviceError:
      "Säätietopalvelu ei vastaa (virhe {code}). Yritetään uudelleen pian.",
    networkError: "Verkkovirhe. Tarkista yhteytesi.",
    cookieBanner:
      "Tämä sivusto käyttää istuntoevästettä tallentamaan asetuksesi. Jatkamalla käyttöä hyväksyt evästeen käytön.",
  ...
  en: {
    ...
    rainSum24h: "Rain 24 h",
    rainSumTotal: "Rain {h} h",
    cameraLoaded: "Loaded",
    cameraDirection: "Direction",
    cameraImageUnavailable: "Image unavailable",
    stationDistanceLabel: "Distance to your location:",
    stationDistanceAccuracy: "(accuracy ±{d})",
    cameraDistanceLabel: "Camera distance from station:",
    mruRecent: "Recent",
    mruAll: "All stations",
    save: "Save",
    cancel: "Cancel",
    ...
    serviceError:
      "Weather data service unavailable (error {code}). Retrying soon.",
    networkError: "Network error. Check your connection.",
    cookieBanner:
      "This site uses a session cookie to remember your settings. By continuing to use this site, you accept its use.",
  ...
  sv: {
    ...
    rainSum24h: "Regn 24 h",
    rainSumTotal: "Regn {h} h",
    cameraLoaded: "Laddad",
    cameraDirection: "Riktning",
    cameraImageUnavailable: "Bild ej tillgänglig",
    stationDistanceLabel: "Avstånd till din plats:",
    stationDistanceAccuracy: "(noggrannhet ±{d})",
    cameraDistanceLabel: "Kamerans avstånd från stationen:",
    mruRecent: "Senaste",
    ...
    serviceError:
      "Väderdatatjänsten svarar inte (fel {code}). Försöker igen snart.",
    networkError: "Nätverksfel. Kontrollera din anslutning.",
    cookieBanner:
      "Den här webbplatsen använder en sessionscookie för att spara dina inställningar. Genom att fortsätta godkänner du användningen.",
...
export function renderWeather(data) {
  ...
  if (hasPW) {
    ...
    setText(
      dom.pwLabel,
      data.present_weather_is_precipitation ? L.precipitation : L.weather,
    );
...
export function forecastGoTo(i) {
  ...
  for (const f of page) {
    ...
    item.className = f.daily
      ? "forecast-item forecast-item--daily"
      : "forecast-item";
    item.innerHTML = `
...
export function populateStations(stations) {
  ...
  const mruStations = mruIds.map((id) => byId.get(id)).filter(Boolean) as any[];

  if (mruStations.length > 0) {
    const mruLabel = labels().mruRecent;
    const allLabel = labels().mruAll;
    const currentInMru = mruStations.some(
...
export function renderStationSearchResults(selectFn) {
  ...
  const matches = query
    ? state.stations.filter((s) =>
        s.formatted_name.toLowerCase().includes(query),
      )
    : state.stations;
