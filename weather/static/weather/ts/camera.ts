...
export function carouselGoTo(i) {
  ...
  if (slide) {
    ...
    const timeStr = now.toLocaleTimeString(
      _state.lang === "en" ? "en-GB" : _state.lang === "sv" ? "sv-SE" : "fi-FI",
    );
    const loadedStr = `${L.cameraLoaded} ${timeStr}`;
    _dom.cameraUpdated.textContent = slide.loaded
      ? slide.presentationName
        ? `${loadedStr} · ${L.cameraDirection} ${slide.presentationName}`
        : loadedStr
      : slide.presentationName
        ? `${L.cameraDirection} ${slide.presentationName}`
        : "";
  }
}
...
export async function showCameraForStation(stationId) {
  ...
  let presets = (feature.properties.presets || []).filter(
    (p) => p.inCollection,
  );
  try {
    ...
    if (dr.ok) {
      ...
      const detailMap = new Map<string, any>(
        (detail.properties?.presets || []).map((p) => [p.id, p]),
      );
      presets = presets.map((p) => ({ ...p, ...(detailMap.get(p.id) || {}) }));
    }
  } catch (_) {
    /* use presets without presentationName */
  }
