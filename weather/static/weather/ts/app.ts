...
async function onSettingsSave() {
  ...
  const historyHours = Math.min(
    24,
    Math.max(1, parseInt(dom.historyHoursInput.value, 10) || 24),
  );
...
function initEvents() {
  ...
  dom.langBtn.addEventListener("click", (e) => {
    ...
    dom.langDropdown.setAttribute(
      "aria-expanded",
      String(dom.langDropdown.classList.contains("open")),
    );
  });
  ...
  dom.forecastNext.addEventListener("click", () => {
    if (
      forecastCarousel.index + FORECAST_PAGE_SIZE <
  ...
  document.addEventListener("fullscreenchange", () => {
    ...
    dom.lightboxFullscreen.setAttribute(
      "aria-label",
  ...
  dom.lightboxTrack.addEventListener(
    "touchstart",
    (e) => {
  ...
  dom.stationLocateBtn.addEventListener("click", () =>
    selectNearestByGeolocation(state.stations),
  );

  dom.stationSearchBtn.addEventListener("click", () =>
    openStationSearch(selectStation),
  );
  ...
  dom.stationSearchInput.addEventListener("input", () =>
    renderStationSearchResults(selectStation),
  );
...
function initCookieBanner() {
  ...
  dom.cookieBannerOk.addEventListener(
    "click",
    () => {
