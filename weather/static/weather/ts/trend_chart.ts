...
export function renderTrendChart(historyData) {
  ...
  if (has_precipitation) {
    ...
    const precipData = precip_series.map((b) => ({
      x: b.time,
      y: b.precipitation,
    }));
    datasets.push({
...
function renderRainSummary(precipSeries, hasPrecipitation, rainSum24h) {
  ...
  document.getElementById("trend-summary-24h-value").textContent =
    `${rainSum24h.toFixed(1)} mm`;
  document.getElementById("trend-summary-24h-label").textContent =
    labels().rainSum24h;
  item24h.classList.remove("hidden");

  const shownHours = precipSeries.length;
  if (shownHours > 0 && shownHours < 24) {
    const shownSum = precipSeries.reduce(
      (acc, b) => acc + (b.precipitation || 0),
      0,
    );
    document.getElementById("trend-summary-total-value").textContent =
      `${shownSum.toFixed(1)} mm`;
    document.getElementById("trend-summary-total-label").textContent =
      labels().rainSumTotal.replace("{h}", shownHours);
