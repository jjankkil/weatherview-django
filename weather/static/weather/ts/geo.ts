...
export async function selectNearestByGeolocation(stations) {
  ...
  const fallback = (reason) => {
    console.warn(
      `[geolocation ${new Date().toISOString()}] Falling back to first station:`,
      reason,
    );
  ...
  try {
    ...
    const position = await new Promise<GeolocationPosition | null>(
      (resolve) => {
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => {
            console.warn(
              `[geolocation ${new Date().toISOString()}] Permission denied or unavailable:`,
              err.message,
            );
            resolve(null);
          },
    ...
    try {
      ...
      console.debug(
        `[geolocation] Got position: lat=${latitude}, lon=${longitude}, accuracy=${accuracy}`,
      );
      ...
      if (!r.ok)
        throw new Error(`nearest-station responded with HTTP ${r.status}`);
      ...
      saveSettings({
        current_station_id: nearest.id,
        current_station_name: nearest.formatted_name || nearest.name,
      });
